...
import { Brain, CalendarDays, CheckCircle2, Target, TrendingUp } from "lucide-react";
...
export function HomeExperience({ isAdmin = false }: { isAdmin?: boolean }) {
            ...
            <div className="grid lg:grid-cols-[1fr_440px]">
              <div className="p-7 md:p-10">
                ...
                <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-600">
                  A plataforma cruza objetivo comercial, contexto, provas e conteúdo do perfil para apontar pontuação, lacunas e próximos passos.
                </p>
                <div className="mt-8 grid gap-3 sm:grid-cols-3">
                  <HeroMetric icon={TrendingUp} label="Pontuação" value="0-100" />
                  <HeroMetric icon={Target} label="Foco" value="Objetivo" />
                  <HeroMetric icon={Brain} label="Inteligência" value="Share AI" />
                </div>
              </div>
              ...
              <aside className="share-green-panel flex min-h-[420px] flex-col justify-between p-7 text-white md:p-8">
                <div>
                  ...
                  <div className="mt-6 grid gap-3 text-sm leading-6 text-white/78">
                    <span className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[var(--share-lime)]" />Login Google para identificar o usuário</span>
                    <span className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[var(--share-lime)]" />Inteligência disponibilizada pela Share AI</span>
                    <span className="inline-flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-[var(--share-lime)]" />Conteúdos e abordagens são entregues como rascunhos para você usar</span>
                  </div>
                </div>
                <div className="mt-8 grid gap-4"><LoginButton label="Acessar cockpit" /></div>
              </aside>
            </div>
          </section>
        </div>
  ...
  return (
    <main className="share-shell min-h-screen text-[var(--share-ink)]">
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-5 py-8">
        <section className="share-green-panel overflow-hidden rounded-lg text-white">
          <div className="grid gap-8 p-6 md:p-8 lg:grid-cols-[1fr_420px]">
            <div>
              ...
              <p className="mt-8 text-sm font-semibold uppercase tracking-wide text-[var(--share-lime)]">Cockpit de autoridade</p>
              <h2 className="mt-3 max-w-3xl text-4xl font-semibold leading-tight md:text-5xl">Transforme posicionamento em execução guiada.</h2>
              <p className="mt-5 max-w-2xl text-lg leading-8 text-white/78">Escolha seu contexto, conecte ou informe seu LinkedIn e receba uma leitura clara sobre autoridade, lacunas e próximos passos.</p>
            </div>
            <div className="grid content-between rounded-lg border border-white/15 bg-white/10 p-5">
              <div>
                <p className="text-sm font-medium text-white/70">Seu diagnóstico</p>
        ...
        <div id="diagnostico"><AuthorityDiagnostic /></div>
      </div>
    </main>
  );
}
