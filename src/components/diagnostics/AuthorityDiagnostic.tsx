  ...
  async function refreshHistory(targetBusinessUnitId = businessUnitId, options: { restoreLatest?: boolean } = {}) {
    try {
      ...
      if (options.restoreLatest) {
        ...
        if (latest) void loadSavedPlan(latest.id);
      }
      return items;
    } catch {
      setHistory([]);
        ...
        {assessment ? <div className="grid gap-6">
          <section className="share-green-panel rounded-lg p-6 text-white">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div><p className="text-xs font-semibold uppercase tracking-wide text-[var(--share-lime)]">Executive Authority Snapshot</p><h3 className="mt-2 text-2xl font-semibold">{authorityClassification}</h3></div>
          ...
          {strategicGaps.length ? <section className="share-card rounded-lg p-5"><h3 className="text-xl font-semibold text-[var(--share-green-950)]">Onde sua autoridade perde força</h3><p className="mt-2 text-sm leading-6 text-zinc-600">Lacunas priorizadas por impacto em percepção e conversa comercial. Ausência de dado aparece como limite de avaliação, não como nota baixa.</p><div className="mt-4 grid gap-4">{strategicGaps.map((gap) => <article key={gap.title} className="rounded-md border border-[var(--share-line)] bg-[#fbfdf8] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-zinc-950">{gap.title}</p><p className="mt-1 text-xs text-zinc-500">Prioridade {gap.priority} · {gap.confidence === "not_evaluated" ? "Não avaliado" : confidenceLabel(gap.confidence)}</p></div><span className="rounded-md bg-white px-2 py-1 text-xs font-semibold text-[var(--share-green-900)]">{gap.nextBestAction}</span></div><div className="mt-4 grid gap-3 md:grid-cols-2"><StrategicGapField label="Diagnóstico" text={gap.diagnosis} /><StrategicGapField label="Leitura especialista" text={gap.expertReading} /><StrategicGapField label="Impacto em autoridade" text={gap.authorityImpact} /><StrategicGapField label="Impacto comercial" text={gap.commercialImpact} /></div>{gap.evidence.length ? <p className="mt-3 text-xs leading-5 text-zinc-500">Evidência: {gap.evidence.join(" · ")}</p> : null}<p className="mt-3 rounded-md bg-white px-3 py-2 text-sm leading-6 text-zinc-700"><strong>Recomendação:</strong> {gap.recommendation}</p></article>)}</div></section> : null}
          ...
          <section className="share-card rounded-lg p-5"><p className="text-xs font-semibold uppercase tracking-wide text-[var(--share-green-800)]">Próxima melhor ação</p><h3 className="mt-1 text-xl font-semibold text-[var(--share-green-950)]">{nextBestAction?.title}</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600">{nextBestAction?.reason}</p><div className="mt-4 grid gap-2 sm:grid-cols-2"><button type="button" onClick={openContentComposer} className="min-h-11 rounded-md border border-[var(--share-green-800)] bg-white px-4 py-2 text-sm font-semibold text-[var(--share-green-900)] hover:bg-[#edf7eb]">Criar conteúdo recomendado</button><button type="button" onClick={generateThirtyDayPlan} disabled={isPlanPending} className="share-button-primary min-h-11 rounded-md px-4 py-2 text-sm font-semibold">{isPlanPending ? "Gerando plano..." : "Gerar plano estratégico de 30 dias"}</button></div>{isPlanPending ? <PlanGenerationStatus /> : null}{actionPanel && actionPanel.eyebrow !== "Sugestão de melhoria" ? <ActionPanelCard panel={actionPanel} /> : null}</section>
...
function legacyAuthorityClassification(score: number) {
