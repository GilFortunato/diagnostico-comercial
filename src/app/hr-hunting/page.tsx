...
import { HrHuntingExperienceV2 } from "@/components/hr-hunting/HrHuntingExperienceV2";
...
export default async function HrHuntingPage() {
  ...
  const accountName = user?.name?.trim() || user?.email?.trim() || null;
  return <><AppHeader isAdmin={isAdminEmail(user?.email)} /><HrHuntingExperienceV2 accountName={accountName} /></>;
}